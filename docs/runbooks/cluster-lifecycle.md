...
| Trigger | New machine; daily start/stop; cluster broken beyond quick repair |
| Impact if ignored | No environment to work in |
| Risk of the procedure | Low (`dev`); cluster data is not preserved by a rebuild |

## Preconditions

- Docker with enough memory for the profile (dev uses about 4 GB); `task tools:check` passes.
- Secret zero available: OS keychain entry, or `TOFU_STATE_PASSPHRASE` exported.
