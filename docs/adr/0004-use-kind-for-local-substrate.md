...
inside Docker on any common host OS, is conformant, supports multiple nodes (to model
node failure) and can be driven from OpenTofu. An initial sketch used one control-plane
plus three workers; to keep what runs routinely small, the daily footprint
was reduced.
