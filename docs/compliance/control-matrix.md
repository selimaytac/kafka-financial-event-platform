...
|---|---|---|---|---|---|
| CM-01 | DORA Art. 9(4)(e) (ICT change management); ISO 27001 A.8.32 | Every change is a reviewed, attributable commit | Git history, Conventional Commits, branch per phase | `git log`; commit-msg hook in `.pre-commit-config.yaml` | implemented |
...
| SEC-01 | ISO 27001 A.5.17 (authentication information) | No secrets committed to the repository | gitleaks pre-commit hook + full-history scan in CI | `task secrets:scan`; `secrets` job in `.github/workflows/ci.yml` | implemented |
